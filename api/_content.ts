...
function getAllSectionIds(toc: TocTree): Set<string> {
  ...
  for (const g of toc.groups) for (const s of g.sections) ids.add(s.id);
  return ids;
}
...
export function canAccessSection(
  ...
  return getAllSectionIds(getTocForTier(tier, primaryPattern)).has(sectionId);
}

export function findSectionById(sectionId: string): Section | null {
  const allToc = getCompleteArchiveToc();
  for (const g of allToc.groups) for (const s of g.sections) if (s.id === sectionId) return s;
  return null;
}
...
export async function getSectionContent(sectionId: string): Promise<{ content: string; readMinutes: number } | null> {
  ...
  try {
    const content = await readFile(join(CONTENT_ROOT, section.filePath), "utf-8");
    return { content, readMinutes: estimateReadTime(content) };
  } catch {
    return null;
  }
}
...
export function getAdjacentSections(
  ...
  for (const g of toc.groups) for (const s of g.sections) allSections.push(s.id);
