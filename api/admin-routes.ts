...
import { testUsers } from "../shared/schema.js";
...
interface TocGroup { id: string; title: string; sections: Section[]; }
interface TocTree { groups: TocGroup[]; }
...
const patternDisplayName: Record<string, string> = {
  disappearing: "The Disappearing Pattern", apologyLoop: "The Apology Loop",
  testing: "The Testing Pattern", attractionToHarm: "Attraction to Harm",
  drainingBond: "The Draining Bond", complimentDeflection: "Compliment Deflection",
  perfectionism: "Perfectionism", successSabotage: "Success Sabotage", rage: "Rage",
};
...
function getCompleteArchiveToc(primaryPattern?: string): TocTree {
  const allPatternGroups: TocGroup[] = Object.entries(patternKeyToDirMap).map(([key]) => {
    const name = patternDisplayName[key] || key;
    const isUserPattern = key === primaryPattern;
    return { id: `pattern-${key}`, title: isUserPattern ? `${name} (yours)` : name, sections: getPatternSections(key) };
  });
  return {
    groups: [
...
function generateAdminToken(): string {
  ...
  adminTokens.set(token, { expires: Date.now() + 60 * 60 * 1000 });
  return token;
}
