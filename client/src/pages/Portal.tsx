...
export default function Portal() {
  ...
  const [userPattern, setUserPattern] = useState<PatternKey>('disappearing');
  ...
  const hasQuickStart = isGodMode;
  const hasArchive = isGodMode;
  const hasAIAccess = isGodMode;
        ...
        <header className="border-b border-white/10 bg-black/50 backdrop-blur-xl sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <div className="flex items-center gap-3">
                {isGodMode && (
                  <span className="px-3 py-1 bg-pink-500/20 border border-pink-500/30 rounded-full text-pink-400 text-xs font-bold">
                    GOD MODE
                  </span>
                )}
                <button
