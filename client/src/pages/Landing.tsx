  ...
  return (
    <div ref={pageRef} className="min-h-screen thread-page" style={{ background: "#0A0A0A", color: "#F5F5F5", fontFamily: "'Inter', sans-serif", overflowX: "hidden" }}>
      ...
      <section className="min-h-screen flex items-center justify-center relative px-6 hero-section-fade" data-testid="section-hero" style={{ overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 1, background: "linear-gradient(to right, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.4) 50%, rgba(0,0,0,0.6) 100%), linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.3) 40%, rgba(0,0,0,0.55) 100%)" }} />
        ...
        <div className="text-center relative z-10" style={{ padding: "80px 24px", position: "absolute", left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: "48rem", boxSizing: "border-box" }}>
          ...
          {(["\u201CYou\u2019ve watched yourself do it.\u201D", "\u201CYou\u2019ve tried to stop.\u201D", "\u201CYou did it anyway.\u201D"] as const).map((line, i) => (<p key={i} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "clamp(0.9rem, 2vw, 1.05rem)", color: "rgba(250,250,250,0.65)", letterSpacing: "0.02em", lineHeight: 1.9, marginBottom: "4px", opacity: 0, animation: `heroRevealUp 0.7s ease ${1.8 + i * 0.8}s forwards` }}>{line}</p>))}
          <div style={{ display: "flex", justifyContent: "center", margin: "28px 0" }}><div style={{ height: "1px", width: "40px", background: "#14B8A6", opacity: 0, animation: "heroFadeIn 0.4s ease 4.5s forwards" }} /></div>
          {(["THAT\u2019S NOT WHO YOU ARE.", "THAT\u2019S A PATTERN RUNNING."] as const).map((line, i) => (<p key={i} style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(2.2rem, 5.5vw, 4rem)", color: "#FAFAFA", fontWeight: 700, letterSpacing: "0.04em", lineHeight: 1.1, marginBottom: "4px", opacity: 0, clipPath: "inset(100% 0 0 0)", WebkitClipPath: "inset(100% 0 0 0)", animation: `heroClipReveal 0.9s cubic-bezier(0.33,1,0.68,1) ${5.2 + i * 0.4}s forwards` }}>{line}</p>))}
          ...
          <div style={{ marginBottom: "28px" }}><span data-testid="text-brand-title" style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "clamp(1.6rem, 3.5vw, 2.4rem)", color: "#14B8A6", letterSpacing: "0.04em", opacity: 0, animation: "heroPivotGlow 1.4s ease 7.5s forwards" }}>THE PATTERN HAS A NAME.</span><span style={{ display: "inline-block", width: "0.55em", height: "0.8em", background: "#14B8A6", marginLeft: "6px", verticalAlign: "middle", opacity: 0, animation: "heroFadeIn 0.01s ease 8.0s forwards, heroCursorBlink 0.9s step-end 8.0s infinite" }} aria-hidden="true" /></div>
